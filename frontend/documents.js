...
App.documents = (function () {
	...
	function checkboxView(id) {
		return '<input type="checkbox" class="form-check-input" document-id="' + id + '">';
	}
	...
	function removeFromTable() {		
		...
		$rows.each(function () {
			...
			if ($input.is(':checked')) {
				ids.push($input.attr('document-id'));
			}
		});
		...
		if (attributes == null && ids.length == 0) {
			return;
		}

		var requestOptions = {
			type: 'POST', 
			data: JSON.stringify({service: attributes.service, ids: ids.join(',')}),
			processData: false,
			contentType: false,
		...
		.then(function (response) {
			for (var id in response) {
				$('[document-id="' + id + '"]').parent().parent().remove();
			}
	...
	function setIdsInTable(response) {
		for (var i = 0, n = response.length; i < n; i++) {
			...
			$cell.parent().find('.form-check-input').attr('document-id', response[i].id);
		}
	}
